...
def download_and_convert_tifgzip_to_jpg(url, target_path, image_size=1024):
    """Function to convert .tif.gz to .png and put it in the same folder
    Eg. in Kaggle notebook
    """
    r = requests.get(url)
...
def download_and_convert_jpg(url, target_path, image_size=1024):
    """Function to convert .tif.gz to .png and put it in the same folder
    Eg. in Kaggle notebook
    """
    r = requests.get(url)
        ...
        for color in colors:
            try:
                ...
                save_path = os.path.join(
                    save_dir, f"{os.path.basename(img)}_{color}.jpg"
                )
                download_and_convert_jpg(img_url, save_path, image_size)
            except:
                img_url = f"{img}_{color}.tif.gz"
                save_path = os.path.join(
                    save_dir, f"{os.path.basename(img)}_{color}.jpg"
                )
                download_and_convert_tifgzip_to_jpg(img_url, save_path, image_size)
